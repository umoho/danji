...
fn stage_gain(sim: &mut Simulator, freq: f64, sr: u32, amplitude: f32) -> f64 {
    ...
    let input: Vec<f32> = (0..n)
        .map(|i| (2.0 * std::f64::consts::PI * freq * i as f64 / sr as f64).sin() as f32 * amplitude)
        .collect();
    let mut out = vec![0.0f32; n];
    sim.process_buffer(&input, &mut out).unwrap();
    let dc: f32 = out.iter().sum::<f32>() / out.len() as f32;
    let rms: f64 = out.iter().map(|x| ((x - dc) * (x - dc)) as f64).sum::<f64>() / n as f64;
...
fn main() -> Result<(), danji::DanjiError> {
    ...
    for _ in 0..2000 { std_amp.process_sample(0.0)?; }
    ...
    for _ in 0..2000 { nc_amp.process_sample(0.0)?; }
    ...
    for _ in 0..2000 { hf_amp.process_sample(0.0)?; }

    println!("=== 12AX7 Frequency Response ===");
    println!("{:>8} {:>10} {:>15} {:>10}", "Freq", "std(dB)", "no-bypass(dB)", "small-cap(dB)");
