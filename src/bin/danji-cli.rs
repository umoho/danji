use danji::{single_triode_config, SimConfig, Simulator, TriodeParams, NodeId, DiodeParams};
use hound::{WavReader, WavSpec, WavWriter};
use std::path::PathBuf;
use clap::Parser;

#[derive(Parser)]
#[command(name = "danji-cli", about = "Tube amplifier simulator - offline audio processor")]
struct Args {
    /// Input WAV file
    #[arg(short, long)]
    input: PathBuf,
...
fn build_two_stage() -> (Simulator, Simulator) {
    ...
    for _ in 0..3000 { s1.process_sample(0.0).unwrap(); }
    for _ in 0..3000 { s2.process_sample(0.0).unwrap(); }
    (s1, s2)
}
...
fn build_chain() -> (Vec<f32>, Simulator, Simulator, Simulator) {
    ...
    p.add_diode(a, b1, 0).add_capacitor(b1, g, 47e-6)
     .add_resistor(b1, bp, 100.0).add_capacitor(bp, g, 47e-6)
     .add_resistor(bp, g, 220e3).input(a).output(bp);
    ...
    for _ in 0..3000 { s1.set_bplus(300.0); s1.process_sample(0.0).unwrap(); }
    for _ in 0..3000 { s2.set_bplus(300.0); s2.process_sample(0.0).unwrap(); }
    (bp_v, s1, s2, {
        let mut t = SimConfig::new(sr, 4);
        let (ti, tm, to) = (NodeId(1), NodeId(2), NodeId(3));
        t.add_resistor(ti, tm, 100_000.0).add_capacitor(tm, g, 330e-12)
         .add_capacitor(ti, to, 0.022e-6).add_resistor(to, g, 100_000.0)
         .input(ti).output(to);
...
fn process(args: &Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if spec.sample_rate != 44100 {
        eprintln!("Warning: sample rate {} Hz, 44.1kHz expected", spec.sample_rate);
    }
    ...
    eprintln!("Input: {} samples, {:.1}s", samples.len(), samples.len() as f64 / spec.sample_rate as f64);
    eprintln!("Model: {}, B+={}V, gain={}dB", args.model, args.bplus, args.gain);

    let dry: Vec<f32> = samples.iter().map(|s| s * args.mix + samples[samples.len().min(0)] * (1.0 - args.mix)).collect();
    drop(dry); // not used yet

    let output: Vec<f32> = match args.model.as_str() {
        "single" => {
            let cfg = single_triode_config(spec.sample_rate, 100_000.0, 1_500.0, 22e-6, 1_000_000.0, args.bplus);
        ...
        "chain" => {
            ...
            samples.iter().enumerate().map(|(i, &s)| {
                ...
                s1.set_bplus(bp); s2.set_bplus(bp);
                let v1 = s1.process_sample(s).unwrap_or(0.0) as f64;
                let a1 = v1 - c1; c1 += a * (v1 - c1);
                let vt = tone.process_sample(a1 as f32).unwrap_or(0.0) as f64;
                let a2 = vt - c2; c2 += a * (vt - c2);
                s2.process_sample(a2 as f32).unwrap_or(0.0)
            }).collect()
        }
        _ => { eprintln!("Unknown model"); return Ok(()); }
    };
    ...
    eprintln!("DC offset: {:.0}V, AC peak: {:.1}V, scale: {:.3}", dc_offset, max_ac, scale);
